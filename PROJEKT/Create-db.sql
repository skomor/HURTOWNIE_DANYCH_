...
nowe_zgony bigint, 
nowe_wzydrowienia bigint,
dzien_od_pierwszego_zakazenia int
);
GO
...
create table STAN_DIM (
id char not null primary key,
opis varchar(255)
);
GO

insert into STAN_DIM values ('p', 'potwierdzony');
insert into STAN_DIM values ('z', 'zgon');
insert into STAN_DIM values ('w', 'wyzdrowia³y');
GO

create table PACJENT_FACT (
id bigint identity(1,1) not null primary key,
data_zachorowania varchar(255) foreign key references CZAS_DIM(data),
data_zmiany_stanu varchar(255) foreign key references CZAS_DIM(data),
kraj varchar(255) foreign key references GEOGRAFIA_DIM(kraj),
id_pacjenta bigint foreign key references PACJENT_DIM(id),
id_stanu char foreign key references STAN_DIM(id)
);
GO
